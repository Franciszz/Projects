...
class SecureRegFTRL(object):

    def __init__(self, L1, L2, alpha, beta, D,
                 L1_fm, L2_fm, fm_dim, fm_initDev, exp_reg,
    ...
    def predict_raw(self, x):
        ...
        """
        if self.exp_reg:
            lambda_weight = lambda a: 1 + exp(- abs(a) - 1)
        else:
            lambda_weight = lambda a: 1 + 1 / (3 + abs(a))
        ...
        if self.interaction:
            ...
            for i in indices:
                ...
                for k in range(self.fm_dim):
                    ...
                    if sign * self.z_fm[i][k] <= self.L1_fm * lambda_weight(self.w[i]):
                        self.w_fm[i][k] = 0.
                    else:
                        # self.w_fm[i][k] = (sign * self.L1_fm - self.z_fm[i][k]) / (
                        #         (self.beta_fm + sqrt(self.n_fm[i][k])) / self.alpha_fm + self.L2_fm)
                        self.w_fm[i][k] = (sign * self.L1_fm * lambda_weight(self.w[i]) - self.z_fm[i][k]) / (
                            (self.beta_fm + sqrt(self.n_fm[i][k])) / self.alpha_fm +
                            self.L2_fm * lambda_weight(self.w[i]))
